  ...
  try {
    ...
    const entries = await db('cashbook')
      ...
      .where({
        ...
        'cashbook.next_reminder_date': '<=' + now
      })
    ...
    for (const entry of entries) {
      const nextReminderDate = new Date(entry.next_reminder_date);
      ...
      if (nextReminderDate <= new Date()) {
        ...
        nextReminderDate.setDate(nextReminderDate.getDate() + (entry.reminder_interval_days || 7));
