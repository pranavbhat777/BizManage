const { exec } = require('child_process');

const setupTaskScheduler = () => {
  console.log('⏰ Setting up automated reminder scheduler...');
  
  // Create Windows Task Scheduler command
  const command = `schtasks /create /tn "BizManage Auto Reminders" /tr "BizManage Auto Reminders" /sc HOURLY /mo HRLY /f "node "${__dirname}\\auto_reminders.js"`;
  
  exec(command, (error, stdout, stderr) => {
    if (error) {
      console.error('❌ Error setting up task scheduler:', error);
      return;
    }
    
    if (stderr) {
      console.error('❌ Task setup stderr:', stderr);
      return;
    }
    
    console.log('✅ Auto reminder scheduler setup successfully!');
    console.log('📋 Task will run every hour');
    console.log('🔧 To manage tasks: schtasks');
    console.log('📱 Manual test: node auto_reminders.js');
    
    // Test the scheduler once
    console.log('🧪 Testing scheduler...');
    exec('node "${__dirname}\\auto_reminders.js"', (testError, testStdout, testStderr) => {
      if (testError) {
        console.error('❌ Test error:', testError);
      } else {
        console.log('✅ Scheduler test completed successfully!');
      }
    });
  });
};

setupTaskScheduler();
